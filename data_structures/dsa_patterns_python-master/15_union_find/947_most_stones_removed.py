...
class Solution:
    ...
    # We want to count connected components (islands) in the graph
    # In each island we  can collect all the stones but 1
    # Ans will be num of stones - num of connected components

    # What is a graph in this case ?
    ...
    # So we need to identify cols with different vertex nums
    # (either ~y - two's compliment, or 10000+y since range of rows and cols is different)

    # Time = O(n) + O(m*log(n)) - m is num of stones
    def removeStones(self, stones: List[List[int]]) -> int:
        # Time: log(n) - n num of rows/ cols
        def find(i):
