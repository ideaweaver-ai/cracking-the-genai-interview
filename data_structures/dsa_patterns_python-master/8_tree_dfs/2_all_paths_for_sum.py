...
def find_paths(root, sum):
    ...
    find_paths_helper(root, sum, result, [])
...
# Best case - Tree is Linked List
# 1 result candidate of lenght O(N). So total O(N)
def find_paths_helper(root, sum, result, current_path):
    # 1) base case 1
    if root is None:
    ...
    find_paths_helper(root.left, sum - root.val, result, current_path)
    # traverse the right sub-tree
    find_paths_helper(root.right, sum - root.val, result, current_path)
